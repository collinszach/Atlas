...
export function FlightForm({ onSuccess }: { onSuccess: (flightId: string) => void }) {
  const { getToken } = useAuth();
  const { mutateAsync: addFlight, isPending } = useAddFlight();
  ...
  const canEnrich = !!flightNumber && !!departureAt;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 max-w-lg">
      ...
      <div className="flex gap-3 pt-2">
        <Button type="submit" loading={isPending}>Log flight</Button>
      </div>
    </form>
  );
}
